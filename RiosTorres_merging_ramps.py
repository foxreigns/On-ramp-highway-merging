...
N = set(list(range(0,30,1)))
NN = set(list(range(0,30,1)))
slen = (len(NN))/2
...
##t0 = np.random.randint(10,50,(len(N),1))
##t0 = np.reshape(np.sort(t0,axis=None),(len(N),1))
#t0 = random.sample(range(10,60),len(N))
#t0 = sorted(t0)
#t0 = np.reshape(np.array(t0),(len(N),1))
tf = np.zeros((len(N),1))
tm = np.zeros((len(N),1))
tm1 = (L)/v0
tm[0] = tm1
...
xTicks = range(0,int(np.ceil(max(tf))),5)
plt.plot(xTicks,L*np.ones(len(xTicks)), color = 'black', linestyle = '--')
plt.plot(xTicks,(L+S)*np.ones(len(xTicks)), color = 'black', linestyle = '--')
...
for ii in N:
    ...
    plt.plot(timePlot[ii], np.array(ppi[ii][:len(timePlot[ii])]))
    #plt.plot(timePlot[ii], np.array(pi[ii][1:]))

    plt.figure(1) # Velocity
    plt.plot(timePlot[ii], np.array(vvi[ii][:len(timePlot[ii])]))
    #plt.plot(timePlot[ii], np.array(vi[ii][1:]))

    plt.figure(2) # Acceleration/Deceleration or Control input
    plt.plot(timePlot[ii], np.array(ui[ii][:len(timePlot[ii])]))
    #plt.plot(timePlot[ii], np.array(ui[ii][:-1]))

    #plt.figure(3) # Acceleration/Deceleration or Control input
    #plt.plot(timePlot[ii], np.array(fuel[ii][:len(timePlot[ii])]))
